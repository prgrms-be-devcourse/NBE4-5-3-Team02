...
export default function ClientPage({postid}: { postid: number }) {
    ...
    useEffect(() => {
        const fetchPostDetail = async () => {
            try {
                const response = await fetchWithAuth(`http://localhost:8080/api/v1/posts/${numericId}`, {
                    method: 'GET',
                    credentials: 'include',
  ...
  const processReservedEvents = (events: any[]) => {
    ...
    const handleSelectSlot = ({ start, end }: SlotInfo) => {
        ...
        const isClickableDate = (date: Date) => {
            return availabilities.some((availability) => {
                ...
                return (
                    moment(date).isSame(startDate, "day") || moment(date).isSame(endDate, "day")
                );
            });
        };
    ...
    const dayPropGetter = (date: Date) => {
        ...
        const isClickable = availabilities.some(availability => {
            const startDate = moment(availability.startTime).startOf("day");
            const endDate = moment(availability.endTime).startOf("day");
            return moment(date).isSame(startDate, "day") || moment(date).isSame(endDate, "day");
        });
            ...
            <div className="w-full max-w-6xl mx-4 bg-white rounded-2xl shadow-2xl overflow-hidden transition-all duration-300 hover:shadow-3xl">
                <Calendar
                    ...
                    focus:outline-none focus:ring-2 focus:ring-green-500/30"
                        ...
                        event: ({ event }) => (
                            <div className={`p-2 m-1 rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200 
            ${event.type === 'personal' ? 'bg-green-500' :
                                event.type === 'meeting' ? 'bg-green-400' :
                                    'bg-green-300'} text-white text-sm border-l-4 border-green-700`}>
                        ...
                        week: {
                            header: ({ date }) => (
                                <div className="text-center py-3 bg-green-50 border-r border-green-100 text-gray-600
            ...
            <div className="flex flex-col items-center mt-10 w-full max-w-2xl mx-auto p-6 bg-green-50 rounded-xl border-2 border-green-200 shadow-sm">
                ...
                <div className="w-full mb-8 p-4 bg-green-100 rounded-lg border border-green-200">
                    ...
                    <p className="mt-4 text-gray-600 leading-relaxed">
                        당일 혹은 1일 전 예약으로 확정되지 않을 수 있으니<br />
                        2~3일 전에 예약해주세요.
                    </p>
                </div>
