  ...
  return (
    ...
    >
      ...
      <div className="w-full max-w-3xl mb-6">
        {post?.images.length ? (
          <img src={post.images[0]} alt={post.title} className="w-full h-72 object-cover rounded-xl shadow-md" />
        ) : (
      ...
      <div className="bg-white p-6 rounded-2xl shadow-lg max-w-3xl w-full">
        ...
        <div className="grid grid-cols-2 gap-4 text-gray-600">
          <p><strong className="text-gray-800">품목 종류:</strong> {post?.category}</p>
          <p><strong className="text-gray-800">가격:</strong> {post?.price.toLocaleString()}원 / {post?.priceType}</p>
          <p><strong className="text-gray-800">조회수:</strong> {post?.viewCount}</p>
          <p><strong className="text-gray-800">등록일:</strong> {post?.createdAt ? new Date(post.createdAt).toLocaleDateString() : '등록일 없음'}</p>
        </div>

        {/* ✅ 이용 가능 날짜 테이블 */}
        <div className="mt-6">
          <h2 className="text-xl font-semibold text-gray-800 mb-4">📅 이용 가능 일정</h2>
            ...
            <table className="w-full border-collapse">
              ...
              <tbody>
                ...
                {nonRecurringDates.map((a, index) => (
                  <tr key={`nonrecurring-${index}`} className="text-center border-t">
                    <td className="p-3 border">{a.date ? new Date(a.date).toLocaleDateString() : '미정'}</td>
                ...
                {recurringDates.map((a, index) => (
                  <tr key={`recurring-${index}`} className="text-center border-t bg-yellow-100">
                    <td className="p-3 border">매주 {getDayOfWeek(a.recurrenceDays)} 가능</td>
                    <td className="p-3 border">
                        {a.startTime.split(" ")[1]} ~ {a.endTime.split(" ")[1]}
                    </td>
                  </tr>
