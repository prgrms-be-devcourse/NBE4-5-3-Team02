...
const handleDistrictChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
  ...
  return (
    ...
    >
      <h1 className="text-3xl font-bold text-gray-800 mb-6">🔍 검색된 게시물</h1>
      ...
      <section className="flex justify-center w-full max-w-3xl mb-6">
        ...
        >
          ...
          />
          <button type="submit" className="ml-4 p-3 bg-green-600 rounded-full hover:bg-green-700 transition-colors">
      ...
      {/* 필터 선택 */}
      <section className="flex flex-wrap justify-center gap-4 mb-6 **bg-white rounded-full px-6 py-4 shadow-xl w-full**"> {/* ✅ 바깥쪽 테두리 제거 */}
        {/* 지역 선택 */}
        ...
        <select value={distance} onChange={(e) => setDistance(e.target.value)}
          ...
          {distanceOptions.map((r) => (
            <option key={r} value={r} className="text-gray-700 hover:bg-gray-100">{r}km</option>
          ))}
        </select>
        ...
        <select value={priceType} onChange={(e) => setPriceType(e.target.value)}
                className="p-2 border rounded-full focus:ring-2 focus:ring-green-500 focus:border-green-500 text-gray-700 shadow-sm bg-white">
          <option value="" className="text-gray-500">가격 타입</option>
          {priceTypeOptions.map((type) => (
            <option key={type} value={type} className="text-gray-700 hover:bg-gray-100">{type}</option>
          ))}
        </select>

        {/* 카테고리 선택 */}
        <select value={category} onChange={handleCategoryChange} className="p-2 border rounded-full focus:ring-2 focus:ring-green-500 focus:border-green-500 text-gray-700 shadow-sm bg-white">
          <option value="" className="text-gray-500">카테고리</option>
          {categoryOptions.map((cat) => (
            <option key={cat} value={cat} className="text-gray-700 hover:bg-gray-100">{cat}</option>
          ))}
        </select>
      </section>
      ...
      <div className="w-full max-w-4xl grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {posts.length > 0 ? (
          posts.map((post) => (
            ...
            >
              ...
              <div className="mt-3 flex justify-between items-center text-gray-500 text-sm">
                <span className="bg-blue-100 text-blue-700 px-2 py-1 rounded-md text-xs">{post.category}</span>
                <span className="font-semibold text-gray-700">{post.price.toLocaleString()}원/{post.priceType}</span>
              </div>

              {/* 조회수 및 등록일 */}
              <div className="mt-3 flex justify-between text-gray-400 text-xs">
                <span>조회수: {post.viewCount}</span>
                <span>등록일: {post.createdAt ? new Date(post.createdAt).toLocaleDateString() : '등록일 없음'}</span>
              </div>
          ...
          <p className="text-gray-600 col-span-full text-center">검색 결과가 없습니다.</p>
        )}
      </div>
    </motion.div>
  );
