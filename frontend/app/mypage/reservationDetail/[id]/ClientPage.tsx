...
import {CalendarDaysIcon, CalendarIcon, CheckCircleIcon, CheckIcon, PencilIcon, StarIcon} from "lucide-react";
...
function RequestedStatus({
  ...
  const confirmAction = async () => {
    ...
    return (
        <div
            className="flex flex-col items-center justify-center w-full max-w-4xl h-auto border border-emerald-200 rounded-2xl shadow-lg p-8 bg-gradient-to-br from-green-50 to-green-100"
        >
            {/* 배경 디자인 요소 추가 */}
          ...
          >
            {renter.nickname}
                            ...
                            <span className="block text-xl mt-2 text-gray-600">님의 예약 요청</span>
                        </h2>
          ...
          <span className="relative flex h-3 w-3">
            ...
            {reservation.amount.toLocaleString()} 원
                        ...
                        <p className="text-xs text-gray-500 mt-2">※ 보증금은 추후 크레딧으로 적립됩니다</p>
                    </div>
                </div>
            </div>
            ...
            overflow-hidden"
                ...
                >
                    {/* 호버 시 채워지는 애니메이션 레이어 */}
                    <div className="absolute inset-0 w-full h-0 bg-emerald-700/90
                transition-all duration-300
                group-hover:h-full ease-out" />

                    {/* 버튼 컨텐츠 */}
                    <div className="relative flex items-center justify-center gap-2">
                        <XMarkIcon className="w-5 h-5 text-emerald-100 transition-transform
            ...
            {showConfirmModal && (
                ...
                >
                    <div className="flex flex-col items-center bg-white p-6 rounded-xl shadow-lg w-full max-w-md border border-emerald-50">
                        {/* 메시지 */}
                        <p className="text-gray-600 text-lg font-medium mb-4">{modalMessage}</p>
            ...
            {showModal && renter && (
                ...
                >
                    <div
                        className="relative p-6 bg-white w-full max-w-md rounded-xl shadow-lg border border-emerald-50">
                        {/* 모달 헤더 */}
                        ...
                        <div className="space-y-3 text-gray-600">
                            <p>
                                ...
                                <span
                                    ...
                                    title="이메일은 호스트에게만 공개됩니다."> {renter.email}
                                    <span
                                        className="absolute invisible group-hover:visible bg-black
                                        text-white text-xs rounded py-1 px-2 top-full left-0"> 이메일은 호스트에게만 공개됩니다.
                                    </span>
                                </span>
                            </p>
                            <p>
                                <b className="text-gray-800">{renter.nickname}</b> 님의 점수는{' '}
                                <span className="text-emerald-600 font-bold">{renter.score}점</span>입니다!
                            </p>
                        </div>
                ...
                <div className="fixed inset-0 flex justify-center items-center bg-black/30 backdrop-blur-sm z-50">
                    <div className="relative bg-white w-11/12 md:w-1/2 lg:w-1/3 p-8 rounded-2xl shadow-2xl border border-emerald-100">
                        ...
                        <div className="space-y-4 text-gray-600">
                            <div>
                                <label className="block text-sm font-medium text-emerald-700">이메일</label>
                                <p className="mt-1">{owner.email}</p>
                            </div>

                            <div>
                                <label className="block text-sm font-medium text-emerald-700">신뢰도 점수</label>
                            ...
                            <div>
                                <label className="block text-sm font-medium text-emerald-700">주소</label>
                                <p className="mt-1">
                            ...
                            <div>
                                <label className="block text-sm font-medium text-emerald-700">전화번호</label>
        ...
        <div className="flex flex-col items-center justify-center w-full max-w-4xl h-auto rounded-3xl shadow-xl p-8 bg-gradient-to-br from-emerald-50 to-green-100 relative overflow-hidden">
          ...
          {formatDate(reservation.startTime)} ~ {formatDate(reservation.endTime)}
            ...
            <div className="mt-8 text-center space-y-4">
                <p className="text-lg font-medium text-gray-600">이용 중 문제가 있으신가요?</p>
                <button
            ...
            {showConfirmModal && (
                <div className="fixed inset-0 flex justify-center items-center bg-black/30 backdrop-blur-sm z-50">
                    <div className="bg-white w-11/12 md:w-1/2 lg:w-1/3 p-6 rounded-2xl shadow-xl border border-emerald-100">
                        <h3 className="text-xl font-bold text-emerald-800 mb-4">문제 유형 선택</h3>

                        <select
                            value={selectedIssue?.value || ""}
                            onChange={(e) => {
                                const option = issueOptions.find(opt => opt.value === e.target.value);
        ...
        <div className="flex flex-col items-center justify-center w-full max-w-4xl h-auto rounded-3xl shadow-xl p-8 bg-gradient-to-br from-emerald-50 to-green-100 relative overflow-hidden">
          ...
          >
            {renter.nickname}
                            ...
                            <span className="block text-xl mt-2 text-gray-600">님의 예약 취소 완료</span>
                        </h2>
                    </div>
                ) : me.id === reservation.renterId ? (
                    <div className="space-y-2">
                        <h2 className="text-3xl font-bold text-gray-800">예약이 취소되었습니다</h2>
            ...
            <div className="w-full bg-white rounded-xl shadow-lg border border-emerald-50 divide-y divide-emerald-100/50">
                <div className="p-6 space-y-4">
                    <div className="bg-red-50/50 rounded-lg p-4 border border-red-100">
                        ...
                        <p className="text-lg text-gray-800 mt-1">{reservation.rejectionReason}</p>
                    </div>
    ...
    return (
        <div className="flex flex-col items-center justify-center w-full max-w-4xl h-auto rounded-3xl shadow-xl p-8 bg-gradient-to-br from-red-50 to-orange-50 relative overflow-hidden">
            ...
            <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-4 w-full max-w-sm">
                ...
                <button
                    className="p-4 rounded-xl bg-red-100 text-red-600 font-medium hover:bg-red-200 transition-all"
                >
                    고객센터 문의
                </button>
            </div>
          ...
          {formatDate(reservation.startTime)} ~ {formatDate(reservation.endTime)}
            ...
            <div className="mt-6 text-center space-y-2">
                <p className="text-sm text-gray-500">
                    보증금 환급 상태: <span className="font-medium text-emerald-600">완료</span>
                </p>
                <p className="text-sm text-gray-500">
                    다음 이용 시 <span className="text-emerald-600">5% 추가 할인</span> 적용
                </p>
            </div>
        </div>
    );
    ...
    return (
        <div className="flex flex-col items-center justify-center w-full max-w-2xl rounded-3xl shadow-xl p-8 bg-gradient-to-br from-emerald-50 to-green-50 relative overflow-hidden">
            ...
            <div className="text-center mb-8 space-y-6">
                ...
                <h2 className="text-4xl font-bold text-gray-800">예약이 취소되었습니다</h2>
            </div>
            ...
            <div className="w-full bg-white rounded-xl shadow-lg border border-emerald-50 divide-y divide-emerald-100/50">
                <div className="p-6 space-y-4">
                    <div className="bg-red-50/50 rounded-lg p-4 border border-red-100">
                        ...
                        <p className="text-lg text-gray-800 mt-1">완전히 처리된 예약 취소</p>
                    </div>

                    <div className="mt-4 space-y-2">
                        <p className="text-sm text-gray-500">예약 ID</p>
                        <p className="text-lg font-medium text-gray-800">{reservation.id}</p>
                    </div>
                </div>
